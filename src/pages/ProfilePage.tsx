      ...
      <div className="w-full max-w-6xl mx-auto px-4 py-12">
        ...
        >
          ...
          <TabsList className="md:w-80 w-full flex-shrink-0 flex flex-col items-stretch p-0 rounded-none">
            <motion.div
              layout
              className="glass-card shadow-2xl rounded-3xl overflow-hidden border border-white/30 bg-gradient-to-br from-white/70 via-glass-100/60 to-green-100/40"
            >
              <div className="p-8 text-center border-b border-white/30">
                <motion.div
                  layout
                  className="relative mx-auto h-28 w-28 rounded-full bg-gradient-to-br from-green-100/60 via-white/40 to-green-200/40 overflow-hidden shadow-lg mb-4 border-4 border-white"
                >
                  <AnimatePresence>
                    {avatar ? (
                      <motion.img
                      ...
                      >
                        ...
                        <span className="absolute bottom-2 left-1/2 -translate-x-1/2 text-xs text-gray-500 bg-white/80 rounded-full px-2 py-1 shadow">
                          No picture
                        </span>
                      </motion.div>
                    )}
                  ...
                  <label
                    ...
                    className="absolute bottom-0 right-0 bg-white rounded-full p-2 shadow-md cursor-pointer hover:bg-gray-50 border border-glass-200 flex items-center gap-1"
                  >
                  ...
                  {avatarError && (
                    <div className="absolute left-1/2 -bottom-8 transform -translate-x-1/2 text-xs text-red-500 bg-white/80 rounded px-2 py-1 shadow">
                      {avatarError}
                    </div>
                  )}
                </motion.div>
                <h3 className="mt-2 text-2xl font-bold text-glass-900">
                  {user?.fullName || "Guest"}
                </h3>
              ...
              <TabsTrigger
                value="profile"
                className="w-full px-8 py-4 text-left border-b border-white/30 rounded-none data-[state=active]:bg-gradient-to-r data-[state=active]:from-green-100/60 data-[state=active]:to-white/60 data-[state=active]:text-green-700 data-[state=active]:shadow-none font-semibold hover:bg-glass-200/40 transition-all"
              >
              ...
              <div className="bg-gradient-to-r from-green-100/60 via-white/40 to-green-200/40 p-4 rounded-b-3xl shadow mt-2">
                ...
                <p className="mt-1 text-sm text-green-700">
                  Enjoy free shipping and exclusive discounts.
                </p>
              </div>
            </motion.div>
          </TabsList>
          ...
          <div className="flex-1">
            ...
            <TabsContent value="profile" className="mt-0">
              <motion.div
                layout
                className="glass-card shadow-xl rounded-3xl overflow-hidden p-8 bg-gradient-to-br from-white/60 via-white/30 to-green-100/40"
              >
                <div className="flex justify-between items-center mb-6">
                  ...
                  <button
                    className="btn-water px-4 py-2 rounded-full font-semibold shadow flex items-center gap-2 hover:scale-105 transition-transform"
                    onClick={openEdit}
                  >
              ...
              >
                <div className="flex justify-between items-center mb-6">
                  ...
                  <div className="flex gap-2">
                    ...
                    />
                    <button
                      className="btn-water px-3 py-2 rounded-full font-semibold shadow flex items-center gap-1"
                      onClick={addAddress}
                    >
                  ...
                  {addresses.length === 0 ? (
                    ...
                    addresses.map((addr: string, idx: number) => (
                      <motion.div
                        key={idx}
                        layout
                        className="flex items-center justify-between bg-white/40 rounded-xl px-4 py-2 shadow"
                      >
                        <span>{addr}</span>
                        <button
            ...
            <TabsContent value="orders" className="mt-0">
              <motion.div
                ...
                className="glass-card shadow-xl rounded-3xl overflow-hidden p-8 bg-gradient-to-br from-white/60 via-white/30 to-green-100/40"
              >
                <h2 className="text-2xl font-bold mb-6 text-glass-900">
                  Order History
                </h2>
                <div className="mb-4 space-y-2">
                  ...
                  <div className="bg-white/40 rounded-xl px-4 py-2 shadow flex items-center justify-between">
                    ...
                    <span className="text-yellow-600 font-semibold flex items-center gap-1">
                      <CreditCard className="h-4 w-4" /> Processing
                    </span>
                  </div>
                </div>
                  ...
                  {notif.length === 0 ? (
                    ...
                    notif.map((n: string, idx: number) => (
                      <motion.div
                        key={idx}
                        layout
                        className="bg-white/40 rounded-xl px-4 py-2 shadow flex items-center gap-2"
                      >
                      ...
                      >
                        ...
                        <div className="mb-3">
                          ...
                          <input
                            ...
                            className="w-full px-3 py-2 rounded-lg border border-glass-200 focus:outline-none focus:ring-2 focus:ring-green-200 text-black"
                          />
                        </div>
                        <div className="flex gap-3 mt-6">
                          <button
                            className="btn-water px-4 py-2 rounded-full font-semibold shadow"
                        ...
                        {passwordChanged && (
                          <div className="mt-4 text-green-700 font-semibold flex items-center gap-2">
                            <CheckCircle className="h-5 w-5" /> Password changed
                            successfully!
                          </div>
                        )}
                      </motion.div>
